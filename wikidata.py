...
class Wikidata:
	...
	def prepare(self, records):
		...
		for record in records:
			if record[ 'links' ][ 'wikidata' ] == None and record[ 'links' ][ 'wikipedia' ] != None:
		...
		for lang in links:
			while len( links[ lang ] ) > 0:
				connections = { **connections, **self.get_ids_from_titles( lang + 'wiki', links[ lang ][:50], lang ) }
	...
	def do_edit(self, entityId, filename, language, lingualibreId):
		response = self.api.request({
			"action": "wbsetclaim",
			"format": "json",
			"claim": '{"type":"statement","mainsnak":{"snaktype":"value","property":"' + PRONUNCIATION_PROPERTY + '","datavalue":{"type":"string","value":"' + filename + '"}},"id":"' + entityId + '$' + str(uuid.uuid4()) + '","qualifiers":{"' + LANG_PROPERTY + '":[{"snaktype":"value","property":"' + LANG_PROPERTY + '","datavalue":{"type":"wikibase-entityid","value":{"id":"' + language + '"}}}]},"references":[{"snaks":{"' + REFURL_PROPERTY + '":[{"snaktype":"value","property":"' + REFURL_PROPERTY + '","datavalue":{"type":"string","value":"https://lingualibre.fr/wiki/' + lingualibreId + '"}}]}}],"rank":"normal"}',
			"summary": SUMMARY,
